package com.foodapp.foodapp.helpers;

public class Constants {
    public static int REVIEW_ACTIVITY_RETURN_CODE = 100;
    public static int CAMERA_PIC_REQUEST = 101;

    public static String API_KEY = "AIzaSyDwH_zuuMFZt9ZHM2fVNhjW-Ux52rVVC5M";

    // Set to 0.1 for testing purposes
    public static double FOOD_PLACE_CONFIDENCE_LEVEL = 0.01;
    public static double FOOD_ITEM_CONFIDENCE_LEVEL = 0.7;

    // Intent extras
    public static String PLACE_EXTRA = "placeName";

    // Notification IDs
    public static int CHECK_PLACE_ID = 150;
}
